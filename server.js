...
const File = mongoose.model("File", filesScheme);
...
app.post('/getSearchList', (req, res) => {
    ...
    // let emails = req.body.users_ids
    // emails.push(req.body.user_id)
    // console.log(emails)
    if(req.body.filters.state_filters.length == 0 && req.body.filters.name_filters.length == 0 && req.body.filters.surname_filters.length == 0 && req.body.filters.gender_filters.length == 0){
        ...
        //     { $sample: { size: 1 } }
        // ])
        // User.find({user_id: { $nin: [req.body.emal]}},{email: 0, _id: 0, password: 0, extra_password: 0, chats: 0, sessionid: 0}).exec(function(err, users) {
        User.aggregate([{ $match: { user_id: { $nin: req.body.users_ids} } }, { $sample: { size: 10}}]).exec(function(err, _users) {
            if (err) throw err;
            let date = new Date()
            ...
            if(_users.length == 0) users[req.body.user_id].emit('search_list', 'no_data')
            for(let i = 0; i < _users.length; i++){
                let age = 0
                if(month < _users[i].age.slice(5,7)) age = year - _users[i].age.slice(0,4) - 1
                else if(month > _users[i].age.slice(5,7)) age = year - _users[i].age.slice(0,4)
                else{
                    if(day >= _users[i].age.slice(8)) age = year - _users[i].age.slice(0,4)
                    else age = year - _users[i].age.slice(0,4) - 1
                }
                console.log(age)
                if(age >= req.body.filters.age_filters_min && age <= req.body.filters.age_filters_max){
                    users[req.body.user_id].emit('search_list', _users[i])
                    send = false
                }
                if(i == _users.length - 1 && send) users[req.body.user_id].emit('search_list', 'no_data')
            }
        });
    }
    else{
        let search = { user_id: { $nin: req.body.users_ids} }
        let cities = []
        ...
        console.log(search)
        User.aggregate([{ $match: search }, { $sample: { size: 10}}]).exec(function(err, _users) {
            if (err) throw err;
            let send = true
            let date = new Date()
            let day = date.getDate()
...
app.post('/uploadVideo', function(req,res){ // добавление видео
    console.log('uploadVideo')
    console.log(req.body.video.length)
    for(let i = 0; i < req.body.video.length; i++){
        const video = new Video({
            video: {contentType: req.body.video[i].type, data: req.body.video[i].file},
        });
        video.save(function(err){
            if(err) console.log(err);
            users[req.body.email].emit('add_system_image', i + 1)
...
app.post('/video', (req, res) => {
    ...
    let n = 0
    async function get(){
        await Video.find().skip( n * 5 ).limit( 5 ).exec(function(err, videos) {
            if (err) throw err;
            for(let i = 0; i < videos.length; i++){
                users[req.headers.email].emit('send_video', videos[i])
            }
            n += 1
            if(videos.length != 0) get()
            // res.send(photos)
        });
    }
    get()
...
app.post('/uploadFile', function(req,res){ // добавление видео
    console.log('uploadFile')
    // res.setHeader('Content-disposition', 'attachment; filename=data.pdf');
    // res.setHeader('Content-type', 'application/pdf');
    //res.send(fs.readFileSync(path.join(__dirname  + '/test_pdf.pdf')))
    // res.sendFile(path.join(__dirname  + '/test.docx'), 'data.pdf', err => {
    //     if (err) console.log(err);
    // });
    for(let i = 0; i < req.body.file.length; i++){
        const { base64encode, base64decode } = require('nodejs-base64');
        // const toPdf = require('office-to-pdf')
        // var pdfBuffer = toPdf( req.body.file[i].file)
       
        // users[req.body.email].emit('send_file', {file: {data: pdfBuffer, contentType: 'application/pdf'}})
        // console.log('2', base64encode(pdfBuffer))
        //console.log(req.body)
        // console.log(pdfBuffer)
       
        // console.log(fs.readFileSync(path.join(__dirname, '/test.docx')))
        if(req.body.file[i].type.lastIndexOf('image') != -1 || req.body.file[i].type.lastIndexOf('audio') != -1 || req.body.file[i].type.lastIndexOf('video') != -1){
            fs.writeFileSync('test_ppt.ppt', req.body.file[i].file)
            users[req.body.email].emit('send_file', {file: {data: req.body.file[i].file, contentType: req.body.file[i].type}})
        }
        else if(req.body.file[i].type == "application/pdf"){
            fs.writeFileSync('test_ppt.ppt', req.body.file[i].file)
            res.send({file: req.body.file[i].file, type: 'application/pdf'})
        }
        else{
            fs.writeFileSync('test_ppt.ppt', Buffer.from(req.body.file[i].file))
            
            const extend = '.pdf'
            const enterPath = path.join(__dirname, '/test_ppt.ppt');
            ...
            libre.convert(file, extend, undefined, (err, done) => {
                ...
                // console.log(done)
                fs.writeFileSync('test_pdf_ppt.pdf', dataurl.convert({ data: done, mimetype: 'application/pdf'}))
                res.send({file: dataurl.convert({ data: done, mimetype: 'application/pdf'}), type: 'application/pdf'})
                
            });
        }
        // const file = new File({
        //     file: {contentType: req.body.file[i].type, data: req.body.file[i].file},
        // });
        // file.save(function(err){
        //     if(err) console.log(err);
        //     users[req.body.email].emit('add_system_file', i + 1)
...
app.post('/file', (req, res) => {
    ...
    async function get(){
        File.find().exec(function(err, files) {
            if (err) throw err;
            // users[req.headers.email].emit('send_file', files.slice(-1)[0])
            // for(let i = 0; i < files.length; i++){
            //     users[req.headers.email].emit('send_file', files[i])
            // }
            // n += 1
...
async function start(){
    try{
        ...
        // Read files
        console.log(typeof fs.readFileSync(path.join(__dirname, '/test.docx')))
        // User.update(
        //     {name: 'Test'},
        //     {password: hash},
        //     {multi: true},
